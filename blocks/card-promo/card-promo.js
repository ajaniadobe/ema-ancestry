export default function init(el) {
  ...
  inner.classList.add('card-promo-inner');
  const pic = el.querySelector('picture');
  if (pic) {
    const picPara = pic.closest('p');
    if (picPara) {
      const picDiv = document.createElement('div');
      picDiv.className = 'card-promo-picture-container';
      picDiv.append(pic);
  ...
  if (!con) return;
  con.classList.add('card-promo-content-container');
  ...
  ctaPara.classList.add('card-promo-cta-container');
