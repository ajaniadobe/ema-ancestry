...
function parse6(element, { document: document2 }) {
  const seen = /* @__PURE__ */ new Set();
  const paragraphs = Array.from(element.querySelectorAll(".cmp-text p")).filter((p) => {
  ...
  const cta = element.querySelector(".cmp-button__wrapper a[href]") || element.querySelector("a[href]");
  ...
  const ctaCell = document2.createElement("div");
  if (cta) {
    const p = document2.createElement("p");
    const a = document2.createElement("a");
    a.href = cta.href;
  ...
  const block = WebImporter.Blocks.createBlock(document2, { name: "banner-promo", cells });
  element.replaceWith(block);
}
...
export {
  import_homepage_default as default
};
